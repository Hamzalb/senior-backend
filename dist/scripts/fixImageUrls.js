    ...
    try {
        ...
        for (const product of products) {
            const fixedImages = product.images.map((url) => {
                ...
                return url.replace('/storage/v1/object/public/images/', '/storage/v1/object/public/sineor/');
            });
            // Check if any URLs were actually changed
            const hasChanges = fixedImages.some((url, index) => url !== product.images[index]);
            if (hasChanges) {
